...
class ProbabilityEngine :
    ...
    def conditionalProbability(self,
        ...
        if self.dataFrame is None :
            raise DataLoadError("Fatal Error! Data Not Loaded. Call 'loadData()' Method First.")
...
def main() :
    inFilePath = r"C:\AI&ML\my-AI-ML-journey\Python-AIML\002_Probability\006_TheoreticalProbability\DataSets\CustomerData.xlsx"
    ...
    try :
        ...
        print(f"\nScenario 01 : p(Churn = Yes | IncomeLevel = High)")
        scenarioProbability01 = probabilityEngine.conditionalProbability(
                                inTargetColumn = "Churn",
                                inTargetValue = "Yes",
                                inGivenColumn = "IncomeLevel",
                                inGivenValue = "High"
                            )
        print(f"P(Churn = Yes | IncomeLevel = High) = {scenarioProbability01 : .4f}")

        print(f"\nScenario 02 : P(SubscriptionType = Premium | IncomeLevel = High)")
        scenarioProbability02 = probabilityEngine.conditionalProbability(
                                inTargetColumn = "SubscriptionType",
                                inTargetValue = "Premium",
                                inGivenColumn = "IncomeLevel",
                                inGivenValue = "High"
                            )
        print(f"P(SubscriptionType = Premium | IncomeLevel = High) = {scenarioProbability02 : .4f}")

        print(f"\nScenario 03 : P(Churn = Yes | SubscriptionType = Basic)")
        scenarioProbability03 = probabilityEngine.conditionalProbability(
                                inTargetColumn = "Churn",
                                inTargetValue = "Yes",
                                inGivenColumn = "SubscriptionType",
                                inGivenValue = "Basic"
                            )
        print(f"P(Churn = Yes | SubscriptionType = Basic) = {scenarioProbability03 : .4f}")

        print(f"\nScenario 04 : P(AgeGroup = Senior | Region = North)")
        scenarioProbability04 = probabilityEngine.conditionalProbability(
                                inTargetColumn = "AgeGroup",
                                inTargetValue = "Senior",
                                inGivenColumn = "Region",
                                inGivenValue = "North"
                            )
        print(f"P(AgeGroup = Senior | Region = North) = {scenarioProbability04 : .4f}")
