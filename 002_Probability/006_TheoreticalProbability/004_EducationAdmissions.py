...
def main() :
    inFilePath = r"C:\AI&ML\my-AI-ML-journey\Python-AIML\002_Probability\006_TheoreticalProbability\DataSets\EducationData.xlsx"
    ...
    try :
        ...
        print(f"\nScenario 01 : p( Graduated = Yes | EnglishProficiency = High)")
        scenarioProbability01 = probabilityEngine.conditionalProbability(
                                inTargetColumn = "Graduated",
                                inTargetValue = "Yes",
                                inGivenColumn = "EnglishProficiency",
                                inGivenValue = "High"
                            )
        print(f"P(Graduated = Yes | EnglishProficiency = High) = {scenarioProbability01 : .4f}")

        print(f"\nScenario 02 : P(ScholarshipAwarded = Yes | CountryRegion = Asia)")
        scenarioProbability02 = probabilityEngine.conditionalProbability(
                                inTargetColumn = "ScholarshipAwarded",
                                inTargetValue = "Yes",
                                inGivenColumn = "CountryRegion",
                                inGivenValue = "Asia"
                            )
        print(f"P(ScholarshipAwarded = Yes | CountryRegion = Asia) = {scenarioProbability02 : .4f}")

        print(f"\nScenario 03 : P(FinancialSupportLevel = Full | ProgramType = PhD)")
        scenarioProbability03 = probabilityEngine.conditionalProbability(
                                inTargetColumn = "FinancialSupportLevel",
                                inTargetValue = "Full",
                                inGivenColumn = "ProgramType",
                                inGivenValue = "PhD"
                            )
        print(f"P(FinancialSupportLevel = Full | ProgramType = PhD) = {scenarioProbability03 : .4f}")

        print(f"\nScenario 04 : P(Graduated = Yes | FinancialSupportLevel = Full)")
        scenarioProbability04 = probabilityEngine.conditionalProbability(
                                inTargetColumn = "Graduated",
                                inTargetValue = "Yes",
                                inGivenColumn = "FinancialSupportLevel",
                                inGivenValue = "Full"
                            )
        print(f"P(Graduated = Yes | FinancialSupportLevel = Full) = {scenarioProbability04 : .4f}")
